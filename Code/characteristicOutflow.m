...
for countK = 2:nsteps
   if ( a_N - countK >= 0 ) 
       ...
       %------------------------------------------
       A_Q_nmk = solQ(countK,1);
   end
   P_Q_nmk  = getPressure(A_Q_nmk,R_end,P0);
   Q_tms    = Q_tms + P_Q_nmk*a_Admittances(countK)*delK;
end

denomTerm = -(Q_R/A_R) + C_R;

nlEqAQ = @(x)x + getPressure(x,R_end,P0)*a_Admittances(1)*(delK/denomTerm) + ...
    (Q_tms/denomTerm) - (Q_R/denomTerm) - (H_R_plus*delK);
...
% Calculate Q_Q:
%---------------
Q_Q = getPressure(A_Q, R_end, P0)*a_Admittances(1)*delK + Q_tms;
Q   = Q_Q;
